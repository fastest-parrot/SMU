...
1.  State the assumptions of the MANOVA and LDA models.
    ...
    - The assumptions for MANOVA are the same as ANOVA:
      
      - Random and independent sampling
      
      - Individual dependent variable residuals are normally distributed amongst population and within groups
      
      - Outliers have been classified and adressed
    
    But for MANOVA - one must also verify that the covariances of each population are equal with respect to the response variables
    
    For LDA the assumptions are the same but if equal covariance assumption is not met, then other techniques (QDA) must be used.
    
      

2.  What is the fundamental difference between MANOVA and LDA since they both share the same underlying assumptions?
    
    - MANOVA is a regression-based technique that seeks to find differences amongst groups with respect to multiple response variables. LDA inverts that process and tries to predict which categorical group an observation belongs to, given a particular set of response variables.
    
3.  What is a confusion matrix? 

    - A confusion matrix allows you to assess your type 1 and type 2 error rates by displaying the rates of TP/FP/TN/FN
  
4.  What is the difference between LDA and QDA, that is, what assumption of the LDA model is relaxed?

    - In LDA discriminate functions are linear with respect to response variables whereas they can be higher order polynomials in QDA
...
### Prediction boundary has shifted significantly towards the red area (NO) which means that the classifier is more likely to predict below the boundary (YES) due to the prior probablities being unbalanced.
...
### Prediction boundary changes dramatically and the observation is clearly an outlier (no other observation comes close to having zero for X2)
...
fields = names(train)[1:length(names(train))-1]

for(x in fields){
  plot(train[,x], col=train$Response,  main=glue('Shift in {x}'))
...
1-ME
```
### Slghtly worse - but not terribly so
