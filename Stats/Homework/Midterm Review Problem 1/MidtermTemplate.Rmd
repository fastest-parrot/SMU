...
```{r}
raw_data = read.csv('prostate.csv')
raw_data[,'gleason'] = factor(raw_data[,'gleason'])
res.aov = aov(psa ~ gleason, data=raw_data)
summary(res.aov)
```
...
group_by(raw_data, gleason) %>% summarise(
  count = n(),
  mean = mean(psa, na.rm = TRUE),
  sd = sd(psa, na.rm = TRUE)
)
ggboxplot(raw_data, x='gleason', y='psa', color = 'gleason',
          palette=palette(rainbow(length(levels(raw_data$gleason)))),
